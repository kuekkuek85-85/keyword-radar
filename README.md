...
`vercel.json` 에서 `0 20 * * *` (UTC) = **매일 05:00 KST**. 시간대는 UTC 기준이므로 원하는 한국 시각 −9시간으로 설정하세요. 아침·저녁 2회로 늘리려면 `crons` 배열에 항목을 추가합니다.

> ⚠️ Vercel Hobby 플랜은 크론 실행에 **±1시간(해당 시(hour) 안 아무 때나) 유동 범위**가 있습니다. 분 단위로 정확히 실행되지 않으며, 이는 Vercel 정책으로 코드에서 조정할 수 없습니다. 분 단위 정밀도가 필요하면 Pro 플랜이 필요합니다.
